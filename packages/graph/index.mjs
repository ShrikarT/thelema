...
function isGraphConfigured(env = process.env) {
  ...
  const hasCross = Boolean(
    env.GRAPH_CROSS_PROTOCOL_ENABLED === "true" || safeId(env.GRAPH_CROSS_SOURCE1_SUBGRAPH_ID) && safeId(env.GRAPH_CROSS_SOURCE1_POOL_ID)
  );
...
var QUERY_STATUS = "Standardized Messari DEX AMM schema queries verified on Uniswap V3 and SushiSwap Arbitrum One deployments.";
export {
  DEFAULT_CROSS_SOURCE1,
  DEFAULT_CROSS_SOURCE2,
  QUERY_STATUS,
  STANDARDIZED_MESSARI_DEX_QUERY,
  analyzeCrossProtocol,
  fetchStandardizedSource,
  getCrossProtocolReferences,
  getReferences,
  isGraphConfigured
};
