...
function ModeSwitch({mode,change}: ModeSwitchProps){return <div className="segmented mode-switch" aria-label="Execution mode"><button aria-pressed={mode==='sandbox'} onClick={()=>change('sandbox')}>Sandbox</button><button aria-pressed={mode==='arc'} disabled={PREVIEW} title={PREVIEW ? 'Arc mode is available in the hosted application.' : undefined} onClick={()=>!PREVIEW&&change('arc')}>Arc Testnet</button></div>;}
...
function formatUtc(secondsOrMs: number | bigint | undefined, fallback: string): string {
  if (!secondsOrMs) return fallback;
  const ms = toTimestampMs(secondsOrMs);
...
function App(){
  ...
  {route==='/'?<Landing Link={Link} stats={mode==='sandbox'?snap?.stats:undefined} faq={<FAQ/>}/>:
  route==='/market'?(PREVIEW&&mode==='arc'?<div className="container app-container"><div className="app-topline"><div className="breadcrumb">Markets <span>/</span> Technology <span>/</span> sNVDA</div><ModeSwitch mode={mode} change={setMode}/></div><div className="panel" style={{padding:'36px 24px',textAlign:'center',margin:'24px 0'}}><PiInfo style={{fontSize:'32px',margin:'0 auto 12px',display:'block',color:'var(--accent)'}}/><h2 style={{fontSize:'20px',marginBottom:'8px'}}>Arc mode is available in the hosted application.</h2><p style={{maxWidth:'520px',margin:'0 auto 20px',color:'var(--muted)',lineHeight:'1.5',fontSize:'14px'}}>The standalone preview file runs an in-memory client-side sandbox. Interacting with live Arc Testnet smart contracts and wallet signing requires running the application server with network RPC connectivity.</p><button className="button primary" onClick={()=>setMode('sandbox')}>Switch back to Sandbox</button></div></div>:<div className="container app-container"><div className="app-topline"><div className="breadcrumb">Markets <span>/</span> Technology <span>/</span> sNVDA</div><ModeSwitch mode={mode} change={setMode}/></div><div className="mode-note"><PiFlask/>{mode==='sandbox'?`Local sandbox · ${PREVIEW?'Runs in this file':'Isolated server session'} · No real funds or blockchain transactions`:cutoffPassed?'Arc Testnet · Demo trading window closed · Market statistics and on-chain records remain viewable in read-only mode':config.arcReady?'Arc Testnet · Active on-chain market · Trade with connected Web3 wallet':'Arc Testnet · Public wallet transactions · Deployment configuration required'}</div><div className="market-heading"><div><div className="market-eyebrow"><span className="asset-tag">sNVDA</span><Status blue={!cutoffPassed&&snap?.status==='open'}>{snap?.status==='settled'?'Settled':snap?.lifecycle==='EVENT_RESOLVED'?'Event resolved':cutoffPassed?'Trading window closed':mode==='sandbox'?'Sandbox market':'Active market'}</Status></div><h1>Will the US allow advanced AI chip<br className="desktop-break"/> sales to China by 31 December 2026?</h1><div className="market-meta"><span>Synthetic Nvidia index</span><span>Event deadline: {formatUtc(snap?.timing?.eventDeadline, '31 Dec 2026 23:59:59 UTC')}</span><span>Trading cutoff: {formatUtc(snap?.timing?.tradingCutoff, '1 Jan 2027 00:59:59 UTC')}</span><span>{money(snap?.stats?.cap ?? 500)} payout cap</span><span>DEMO_ORACLE</span></div></div><div className="balance-box"><span>{mode==='sandbox'?'Sandbox balance':'Wallet USDC'}</span><strong>{money(mode==='sandbox'?snap?.balance:wallet?.balance)}</strong><button className="text-button" onClick={mode==='sandbox'?()=>setReset(true):connect}>{mode==='sandbox'?'Reset sandbox':'Refresh wallet'}<PiArrowClockwise/></button></div></div>{error&&<ErrorBox>{error}</ErrorBox>}<StatsGrid stats={snap?.stats} loading={loading}/><div className="market-grid"><div className="market-main"><section className="panel worlds-panel"><div className="panel-heading"><div><span className="eyebrow">The conditional view</span><h2>Same asset. Different futures.</h2></div><PiGitBranch/></div><Worlds stats={snap?.stats}/><p className="panel-note">Conditional prices come from share midprices divided by outcome probability. They are market signals, not forecasts or guaranteed returns.</p></section><Identity stats={snap?.stats} refs={refs} refresh={refreshRefs}/><MarketTabs {...common}/></div><aside className="trade-column" aria-label="Trade ticket"><Ticket {...common} connect={connect}/><div className="risk-note"><PiInfo/><p><strong>Know the payoff.</strong> Losing shares pay zero. Winning asset shares pay the index up to {money(snap?.stats?.cap ?? 500)}. Remainder is claimable by residual (R) holders.</p></div><div className="mobile-balance"><span>{mode==='sandbox'?'Sandbox balance':'Wallet USDC'}: {money(mode==='sandbox'?snap?.balance:wallet?.balance)}</span>{mode==='sandbox'&&<button className="text-button" onClick={()=>setReset(true)}>Reset sandbox <PiArrowClockwise/></button>}</div></aside></div></div>):
  route==='/portfolio'?(PREVIEW&&mode==='arc'?<div className="container app-container"><div className="app-topline"><span className="eyebrow">Your market exposure</span><ModeSwitch mode={mode} change={setMode}/></div><div className="panel" style={{padding:'36px 24px',textAlign:'center',margin:'24px 0'}}><PiInfo style={{fontSize:'32px',margin:'0 auto 12px',display:'block',color:'var(--accent)'}}/><h2 style={{fontSize:'20px',marginBottom:'8px'}}>Arc mode is available in the hosted application.</h2><p style={{maxWidth:'520px',margin:'0 auto 20px',color:'var(--muted)',lineHeight:'1.5',fontSize:'14px'}}>The standalone preview file runs an in-memory client-side sandbox. Interacting with live Arc Testnet smart contracts and wallet signing requires running the application server with network RPC connectivity.</p><button className="button primary" onClick={()=>setMode('sandbox')}>Switch back to Sandbox</button></div></div>:<div className="container app-container"><div className="app-topline"><span className="eyebrow">Your market exposure</span><ModeSwitch mode={mode} change={setMode}/></div><div className="page-heading"><h1>Your positions.</h1><p>Track each world separately. Claim only after resolution.</p></div><div className="mode-note"><PiFlask/>{mode==='sandbox'?'All positions and activity below are local simulations.':cutoffPassed?'Arc Testnet · Demo trading window closed · Existing on-chain balances and claims are preserved':wallet?'Arc Testnet · Verified on-chain token balances for connected wallet':'Arc Testnet · Connect wallet to view token holdings and claim balances'}</div>{error&&<ErrorBox>{error}</ErrorBox>}<div className="portfolio-layout"><section className="panel"><div className="panel-heading"><h2>Holdings</h2><Status>{mode==='sandbox'?'Local sandbox':'Arc Testnet'}</Status></div><Positions {...common}/></section><Pairs {...common}/></div><section className="panel activity-panel"><div className="panel-heading"><h2>Your activity</h2><span className="muted">Sandbox records, not chain transactions</span></div><Activity snap={snap}/></section><button className="button secondary" onClick={()=>go('/market')}>Back to the market <PiArrowRight/></button></div>):
 ...
 </main><footer className={`footer container ${route==='/'?'landing-footer':''}`}><div><Link to="/" className="brand"><Mark/><span>THELEMA</span></Link><p>Synthetic impact markets.<br/>Testnet only. Unaudited. Not investment advice.</p></div><div className="footer-links"><Link to="/guide">Risks & methodology</Link><External href="https://if.market">Category inspiration</External><span className="muted">No production service or approved legal policies.</span></div><span className="footer-build">Research build / 2026</span></footer>
  ...
  return <section className="panel identity-panel">
    ...
    {cross&&(
      <div style={{margin:'0 24px 20px',padding:'16px',border:'1px solid var(--border)',borderRadius:'8px',background:'var(--soft)'}}>
        ...
        <div className="muted" style={{fontSize:'11px',marginTop:'8px',display:'flex',justifyContent:'space-between',flexWrap:'wrap',gap:'6px'}}>
          <span>{cross.updatedAt ? `Observation: ${new Date(cross.updatedAt).toLocaleTimeString()}` : 'Freshness: verified within max-age bound'}</span>
          <span>External DEX reference context only · Not comparable to sNVDA · Not a settlement oracle</span>
        </div>
      </div>
    )}
...
function Ticket({mode,snap,wallet,config,connect,refresh,notify}: TicketProps){
 ...
 const getReason=()=>{
  ...
  const now=Date.now();
  const cutoffMs = snap.timing?.tradingCutoff ? toTimestampMs(snap.timing.tradingCutoff) : 0;
  if(cutoffMs > 0 && now > cutoffMs)return 'Testnet demo trading window closed';
...
function Pairs({snap,mode,wallet,config,refresh,notify}: PairsProps){
 ...
 return <section className="panel vault-panel"><div className="panel-heading"><h2>Collateral pairs</h2><PiArrowsSplit/></div><p>Mint complete sets with collateral (Y + N for binary, Y + N + R for asset). Merge complete sets before settlement to unlock collateral.</p><label className="field-label" htmlFor="pair-book">Book</label><select id="pair-book" value={book} onChange={e=>setBook(e.target.value as Book)}><option value="binary">Event · $1 per pair</option><option value="asset">Asset · {money(snap?.stats?.cap ?? 500)} per complete set (Y+N+R)</option></select><label className="field-label" htmlFor="pair-quantity">Number of pairs</label><input id="pair-quantity" className="plain-input" inputMode="decimal" value={quantity} onChange={e=>setQuantity(e.target.value)}/><div className="vault-buttons"><button className="button primary" onClick={()=>run('split')} disabled={pairsFrozen}>Split pair</button><button className="button secondary" onClick={()=>run('merge')} disabled={pairsFrozen}>Merge pair</button></div>{pairsFrozen&&<p className="muted" style={{fontSize:'12px',marginTop:'8px'}}>{snap?.status==='settled'?'Market settled. Collateral pairs cannot be minted or merged.':cutoffPassed?'Testnet demo trading window closed. Collateral pairs cannot be minted or merged.':'Pair minting and merging close once the event resolves.'}</p>}{error&&<ErrorBox>{error}</ErrorBox>}<p className="ticket-foot">{mode==='sandbox'?'Simulation only. No real funds.':'Each transaction needs a wallet signature.'}</p></section>;
}
